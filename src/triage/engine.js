...
// `browser` is a Chromium instance owned and closed by the caller (triage()
// below shares one instance across an entire run instead of launching one
// per step - see the "resolve the anchor, capture the current state, prove
// candidates" note in the browser-pooling cleanup). This function only
// manages the page it opens on that browser, loading the BASELINE html
// (with scripts neutralized) rather than navigating to any live url.
async function resolveAnchorPath(browser, baseline, selector) {
...
export async function triage(opts) {
  ...
  // One triage run used to launch up to three separate Chromium instances
  // (resolve the anchor against the baseline, capture the current build,
  // prove candidates) - now a single instance is launched lazily on first
  // use and shared across all of them, then closed once here regardless of
  // which return path is taken. Each step still gets its own page/context,
  // since resolveAnchorPath loads the neutralized BASELINE html while
  // captureSnapshot and proveCandidates navigate to the LIVE currentUrl -
  // sharing the browser process is safe, sharing a page or context is not.
  ...
  async function runTriage() {
      ...
      if (rerun.failures === 0 || rerun.nondeterministic) {
        ...
        if (opts.temporal) {
          ...
          if (!target) {
            ...
            const targetIsValid = await isValidCssTarget(await getBrowser(), target);
            if (!targetIsValid) {
              notes.push(
    ...
    const resolved = await resolveAnchorPath(await getBrowser(), baseline, anchor.selector);
    if (resolved.error) {
    ...
    const current = opts.currentPath
      ...
      : await captureSnapshot(opts.currentUrl, { browser: await getBrowser() });
    ...
    if (verdict === 'fragile') {
      ...
      if (!source) {
        ...
        if (candidates.length === 0) {
          ...
          try {
            recommendation = await proveCandidates(opts.currentUrl, source.path, candidates, { browser: await getBrowser() });
