...
**Known limitation: the acknowledgment channel needs a shared filesystem.** Every "installed"/"acknowledged" claim above - and the blindspots one below - works the same way: the wrapper writes a receipt file from inside the suite process at a path handed to it via an env var (`FLAKEPROOF_TEMPORAL_ACK`, or `FLAKEPROOF_MUTATION_ACK` for blindspots), and flakeproof reads that same path back afterward. That only works when the suite process and flakeproof's own process see the same filesystem. If your rerun/suite command execs into somewhere else - a container (`docker exec app npm test`), a remote host (`ssh ci-runner npm test`), a separate CI job - the wrapper can be installed correctly and still leave flakeproof reporting "the inject wrapper never acknowledged the delay" forever, indistinguishable from a genuinely missing setup, because its ack file lands on a filesystem flakeproof never sees.

There is no such false report from a signal that is already known to survive that same boundary: the exit code and stdout/stderr of whatever local command you hand `--rerun-cmd` (a `docker exec`, an `ssh` call, etc. all relay these back to their local caller, which is exactly what lets flakeproof observe pass/fail through that boundary in the first place). Using that channel to also carry a corroborating "the wrapper did run" marker is possible in principle, but was deliberately not built in this cleanup pass: it would mean every injection adapter (Playwright, Cypress, Selenium, Puppeteer, and the separate Python Robot Framework listener - five independent code paths) emitting that marker, plus `rerunStats` and `temporalProbe` threading it through as new evidence. `src/triage/temporal-probe.js` is already the most heavily revised module in this codebase (see the `Fix 1`-`Fix 7` comments throughout it, each closing a prior review finding); adding a new evidence channel to it belongs in its own reviewed change, not a drive-by inside a tech-debt cleanup. For now: run flakeproof and the command it reruns in the same filesystem namespace, or treat "wrapper not installed" as inconclusive rather than a confirmed negative when you know your setup crosses a filesystem boundary.
...
- **If the wrapper never acknowledges a run, flakeproof refuses to print a score.** A suite that stays green because flakeproof never reached the page is not a blind suite, it is an unmeasured one. The report tells you exactly how to install the wrapper instead - but see the shared-filesystem limitation under "Catching missing waits" above: this same message also fires forever when the wrapper IS installed but its ack file lands on a filesystem flakeproof cannot see (a containerized or remote runner).
...
Shipped and covered by the test suite:

- **Red triage** (five-verdict classification, DOM diffing at the anchor, proven selector recommendations) across five frameworks - Playwright, Robot Framework, Cypress, Selenium and Puppeteer all have an anchor-extraction reader; see "Framework adapters" above for exactly what each one can and cannot resolve.
- **The temporal lane** (provoked-delay reproduction for a suspected missing wait) for all five of those frameworks, including a dedicated Robot Framework listener (issue #11) - see "Catching missing waits" above, and its known shared-filesystem limitation there.
...
Marked opt-in or limited on purpose, not overlooked: the temporal and blindspots wrappers are something the user installs in their own suite (nothing runs without that one-time setup), the ack mechanism behind both needs flakeproof and the suite to share a filesystem (see "Catching missing waits"), and blindspots' semantic-mutation injection, candidate-proving inside the user's own test run, and grading new tests before they enter the suite are all still on the roadmap - not shipped for any framework yet.
